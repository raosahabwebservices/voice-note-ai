...
import { Mic, Mail, Phone, Lock, User, Globe, ArrowRight } from 'lucide-react';
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      if (!email.trim() || !phoneNumber.trim()) {
        setErrorMsg('Both Email Address and Mobile Number are required for registration.');
        setLoading(false);
        return;
      }
    }
    ...
    try {
      ...
      const body = mode === 'register'
        ? { 
            name, 
            email, 
            phone: `${selectedCountry.dial}${phoneNumber.replace(/^0+/, '')}`, 
            password, 
      ...
      <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-8 shadow-2xl relative z-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 text-xs">
            {mode === 'login' && 'Sign in using your Email or Mobile Number'}
            {mode === 'register' && 'Register with both Email and Mobile Number'}
        ...
        {authMethod === 'email' && mode !== 'forgot' && (
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            {mode === 'register' && (
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700">Mobile Number (Required for Registration)</label>
                <div className="flex space-x-2">
                  <select
                    ...
                    className="bg-slate-100 border border-slate-300 rounded-xl px-2 py-3 text-xs text-slate-700 focus:outline-none"
                  >
                    {COUNTRIES.map(c => (
        ...
        {mode === 'forgot' && (
          <form onSubmit={(e) => { e.preventDefault(); setSuccessMsg('Password reset instructions sent to your email.'); }} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-700">Enter your account email or phone</label>
              <input
                type="text"
                required
