...
import { X, Copy, Download, Trash2, CheckCircle2, Circle, Calendar, HelpCircle, FileText, Sparkles, Clock, Check, Volume2, MessageSquare, Send } from 'lucide-react';
import { D3MindMap } from './D3MindMap';
import { StudentStudySuite } from './StudentStudySuite';
...
export const NoteDetailModal: React.FC<NoteDetailModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'keypoints' | 'actions' | 'deadlines' | 'questions' | 'transcript' | 'chat' | 'mindmap' | 'student_suite'>('student_suite');
  const [copied, setCopied] = useState(false);

  // Chat state
  const [messages, setMessages] = useState<Array<{ role: 'user' | 'assistant'; content: string }>>([
    { role: 'assistant', content: `Hi! I'm your AI Study Tutor. Ask me anything about "${note.title}". For example: "Sir ne recursion kaise samjhaya tha?" or "What are the core formulas?"` }
  ]);
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-white text-slate-900 animate-fadeIn overflow-y-auto">
      <div className="w-full max-w-7xl mx-auto flex flex-col min-h-screen bg-white shadow-none">
        ...
        <div className="flex overflow-x-auto px-6 py-2 border-b border-slate-800 bg-slate-950/80 space-x-2">
          {[
            { id: 'student_suite', label: '🎓 Student Study Suite', icon: Sparkles },
            ...
            { id: 'chat', label: '💬 Ask My Lecture', icon: MessageSquare },
          ].map((tab) => {
            const Icon = tab.icon;
            return (
              <button
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {activeTab === 'student_suite' && <StudentStudySuite note={note} token={token} />}
          {activeTab === 'summary' && (
